  ...
  $.customSelect = function (options) {
    ...
    $(options.selector).after(
      "<div id='jqcs_s_" +
        ...
        options.cssClass +
        "'>" +
        "<div class='jqcs_value'><p class='jqcs_placeholder'>" +
        options.placeholder +
        "</p></div>" +
        ...
        "<div id='jqcs_o_" +
        options.identifier +
        "' class='jqcs_options'></div>"
    );

    $("#jqcs_s_" + options.identifier + " .jqcs_arrow").width(
      $("#jqcs_s_" + options.identifier).height()
    );
    ...
    $("#jqcs_s_" + options.identifier).click(function (e) {
      ...
      if ($("#jqcs_o_" + options.identifier).css("display") == "block") {
        ...
        $($("#jqcs_s_" + options.identifier + " .jqcs_arrow")[0]).removeClass(
          "rotated"
        );
      } else {
        $("#jqcs_o_" + options.identifier).slideDown();
        $($("#jqcs_s_" + options.identifier + " .jqcs_arrow")[0]).addClass(
          "rotated"
        );
      }
    });
    ...
    $("#jqcs_o_" + options.identifier + " .jqcs_option").click(function (e) {
      $(`input${options.selector}`)[0].value = $(this).data("select-value");
      $($("#jqcs_s_" + options.identifier + " .jqcs_value")[0]).html(
        this.outerHTML
      );
    });
